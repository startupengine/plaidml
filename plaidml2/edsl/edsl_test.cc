...
TEST(CppEdsl, MnistCnn) {
  ...
  Tensor input(Placeholder(PLAIDML_DATA_FLOAT32, {1, 224, 224, 1}));
  Tensor kernel1(Placeholder(PLAIDML_DATA_FLOAT32, {3, 3, 1, 32}));
  Tensor bias1(Placeholder(PLAIDML_DATA_FLOAT32, {32}));
  auto conv1 = Relu(Convolution2(input, kernel1) + bias1);
  // model.add(Conv2D(64, (3, 3), activation='relu'))
  Tensor kernel2(Placeholder(PLAIDML_DATA_FLOAT32, {3, 3, 32, 64}));
  Tensor bias2(Placeholder(PLAIDML_DATA_FLOAT32, {64}));
  ...
  Tensor kernel3(Placeholder(PLAIDML_DATA_FLOAT32, {12100, 128}));
  Tensor bias3(Placeholder(PLAIDML_DATA_FLOAT32, {128}));
  auto dense1 = Relu(Dot(flat, kernel3) + bias3);
  const std::int64_t kNumClasses = 100;
  // model.add(Dense(num_classes, activation='softmax'))
  Tensor kernel4(Placeholder(PLAIDML_DATA_FLOAT32, {128, kNumClasses}));
  Tensor bias4(Placeholder(PLAIDML_DATA_FLOAT32, {kNumClasses}));
